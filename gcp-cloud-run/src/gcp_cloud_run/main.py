...
class GcpCloudRun:
    ...
    async def _run_crud_test(self, gcloud: dagger.Container, region: str) -> str:
        """Internal: Run CRUD test with a pre-configured gcloud container."""
        results = []
    ...
    async def test_crud(
        ...
        gcloud = dag.gcp_auth().gcloud_container(credentials, project_id, region=region)
        return await self._run_crud_test(gcloud, region)
