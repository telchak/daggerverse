...
class Calver:
    ...
    @function
    async def test_generate_default(self) -> str:
        """Test: Generate version with default YYYY.MM.DD format."""
        version = await self.generate()
        parts = version.split(".")
        if len(parts) != 3:
            msg = f"Expected 3 parts, got {len(parts)}: {version}"
            raise ValueError(msg)
        return f"PASS: {version}"

    @function
    async def test_generate_with_micro(self) -> str:
        """Test: Generate version with manual micro number."""
        version = await self.generate(format="YYYY.MM.MICRO", micro=5)
        if not version.endswith(".5"):
            msg = f"Expected version to end with .5, got {version}"
            raise ValueError(msg)
        return f"PASS: {version}"

    @function
    async def test_generate_custom_format(self) -> str:
        """Test: Generate version with custom prefix format."""
        version = await self.generate(format="v.YY.0M.0D")
        if not version.startswith("v."):
            msg = f"Expected version to start with 'v.', got {version}"
            raise ValueError(msg)
        return f"PASS: {version}"
    ...
    async def test_all(self) -> str:
        """Run all tests."""
        results = []
        results.append(await self.test_generate_default())
        results.append(await self.test_generate_with_micro())
        results.append(await self.test_generate_custom_format())
        return "\n".join(results)
